...
function entryMinutes(log: LogEntry): number {
  return (
    sessionMinutes(log.timeIn, log.timeOut) +
    sessionMinutes(log.pmTimeIn, log.pmTimeOut)
  );
}
...
export default function ActivityPage() {
  ...
  const [otLogs, setOtLogs] = useState<LogEntry[]>([]);
  const [activeTab, setActiveTab] = useState<"daily" | "ot">("daily");
  const [loading, setLoading] = useState(true);
  const [refreshing, setRefreshing] = useState(false);

  const logs = activeTab === "daily" ? dailyLogs : otLogs;
  ...
  const recent = useMemo(() => {
    return [...logs].sort((a, b) => (a.date < b.date ? 1 : -1)).slice(0, 10);
  }, [logs]);
  ...
  const weeklyPct = Math.min(
    100,
    Math.round((stats.weeklyMinutes / (WEEKLY_GOAL_HOURS * 60)) * 100),
  );
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="space-y-2">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-linear-to-br from-orange-500 to-amber-600 flex items-center justify-center shadow-[0_0_20px_rgba(251,146,60,0.35)]">
          ...
          <div className="flex items-center gap-3">
            <Tabs
              value={activeTab}
              onValueChange={(v) => setActiveTab(v as "daily" | "ot")}
              className="w-[200px]"
            >
            ...
            <button
              ...
              className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-zinc-900/80 border border-zinc-800 text-zinc-300 hover:border-orange-500/50 hover:text-orange-500 transition-all disabled:opacity-50 h-10"
            >
              <RefreshCcw
                ...
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="hidden sm:flex items-center gap-1.5 text-[10px] text-zinc-500">
                    ...
                    {[0, 1, 2, 3, 4].map((lvl) => (
                      <div
                        key={lvl}
                        className={cn("w-3 h-3 rounded-sm", LEVEL_STYLES[lvl])}
                      />
                    ))}
                    <span>More</span>
                  </div>
                ...
                <ScrollArea className="h-[360px] pr-3">
                  <div className="space-y-3">
                    {recent.map((log, i) => {
                      ...
                      return (
                        <div key={log.id} className="relative pl-6 pr-1">
                          {/* Timeline line */}
                          {i !== recent.length - 1 && (
                            ...
                            <div className="space-y-1.5">
                              ...
                              {log.pmTimeIn && (
                                <div className="flex items-center gap-2 text-[11px]">
                                  ...
                                  <span className="font-mono text-zinc-400">
                                    {fmtClock(log.pmTimeIn)} –{" "}
                                    {fmtClock(log.pmTimeOut)}
                                  </span>
                                  {log.pmTask && (
                                    <span className="text-zinc-500 truncate">
                                      · {log.pmTask}
...
function KpiCard({
  ...
  return (
    <Card
      className={cn(
        "bg-zinc-900/80 border-zinc-800 p-4 hover:border-zinc-700 transition-colors",
      )}
    >
