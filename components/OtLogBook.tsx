...
export default function OtLogBook() {
  ...
  const [exportType, setExportType] = useState<'excel' | 'pdf' | 'ot-pdf'>('excel');
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      // Convert HTML to PDF locally using html2pdf
  ...
  const openExportModal = (type: 'excel' | 'pdf' | 'ot-pdf') => {
    setExportType(type);
    if (type === 'excel') {
      ...
      <Dialog open={exportModalOpen} onOpenChange={setExportModalOpen}>
        <DialogContent className="bg-zinc-900 border-zinc-800 text-zinc-100">
          ...
          <div className="space-y-4 py-4">
            ...
            {/* Action Options */}
            {(exportType === 'pdf' || exportType === 'ot-pdf') && (
